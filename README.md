...
- **人脸识别与活体防伪**：YuNet 人脸检测 + MobileFaceNet 特征匹配 + MiniFASNetV2 深度学习防伪 + 摩尔纹检测 + 反光检测
- **举手检测**：MediaPipe 姿态估计，检测人员举手行为（求救信号）
- **跌倒检测**：MediaPipe 姿态估计，检测人员跌倒行为
- **火情检测**：基于 HSV 颜色空间和 YOLO 的火情/烟雾检测
- **声学检测**：YAMNet 深度学习模型，检测爆炸声和玻璃破碎声
...
│   │   ├── liveness_detector.py  # 活体检测（含摩尔纹和反光检测）
│   │   ├── hand_raise_detector.py # 举手检测
...
### 8. 声学检测配置
在 `backend/config/audio_detection.json` 中配置声学检测：
```json
{
...
### 9. 人脸欺诈检测配置
在 `backend/core_cv/liveness_detector.py` 中配置防伪参数：
```python
# 决策权重（针对手机屏幕攻击优化）
w1 = 0.50  # MiniFASNetV2 概率
w2 = 0.15  # 拉普拉斯方差（清晰度）
w3 = 0.15  # HSV 颜色分布
w4 = 0.20  # 摩尔纹模式（手机屏幕特征）

# 硬门控阈值
SPOOF_HARD_GATE = 0.35  # P_fas ≤ 0.35 直接判定为 Spoof
LIVE_HARD_GATE = 0.90   # P_fas ≥ 0.90 直接判定为 Live

# 融合阈值
SPOOF_THRESHOLD = 0.30  # Score ≤ 0.30 判定为 Spoof
LIVE_THRESHOLD = 0.40   # Score ≥ 0.40 判定为 Live
```

### 10. 举手检测配置
在 `backend/core_cv/hand_raise_detector.py` 中配置检测参数：
```python
height_threshold = 0.05        # 手腕高于肩膀的阈值
elbow_angle_threshold = 100    # 肘部角度阈值（度）
confidence_threshold = 0.3     # 置信度阈值
confirmation_window = 3        # 确认窗口大小
confirmation_required = 2      # 需要确认的帧数
```
...
| 异常活动告警——举手 | `hand_raise` | 检测到人员举手行为（求救信号） | 闵世宇 |
| 异常活动告警——跌倒 | `fall` | 检测到人员跌倒行为 | 闵世宇 |
| 异常活动告警——火情 | `fire` | 检测到火情或烟雾 | 闵世宇 |
| 声学告警——爆炸声 | `explosion` | 检测到爆炸声 | 闵世宇 |
| 声学告警——玻璃破碎 | `glass_break` | 检测到玻璃破碎声 | 闵世宇 |
...
### Jenkins Pipeline 配置
项目提供完整的 Jenkins CI/CD 流水线配置（`ops/Jenkinsfile`）：

**流水线阶段：**
1. **Frontend Dependencies** - 安装前端依赖（Node.js 20）
2. **Backend Dependencies** - 安装后端依赖（Python 3.11）
3. **Frontend Lint** - 前端代码检查（ESLint）
4. **Frontend Build** - 前端构建（Vite）
5. **Backend Lint** - 后端代码检查（Flake8）
6. **Backend Test** - 后端单元测试（Pytest）
7. **Docker Build** - 构建 Docker 镜像

**使用方法：**
1. 在 Jenkins 中创建 Pipeline 任务
2. 配置 Git 仓库：`git@github.com:li-boss/OmniGuard.git`
3. 选择分支：`combineMonThirNight`
4. 运行构建

**Docker 要求：**
- Jenkins 容器需要挂载 `docker.sock`
- 安装 Docker CLI
