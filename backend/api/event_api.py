...
def _generate_alarm_title(event):
    """根据告警类型和描述生成标题"""
    base_type = event.alarm_type
    if base_type == "electronic_fence":
        base_type = "围栏入侵告警"
    
    # 如果是异常活动告警，根据描述细化
    if event.alarm_type == "异常活动告警" and event.description:
        if "火情" in event.description or "火" in event.description:
            base_type = "异常活动 - 火情"
        elif "跌倒" in event.description:
            base_type = "异常活动 - 跌倒"
    
    return f"{base_type} - 摄像头 {event.camera_id}"
...
def serialize_event(event):
    ...
    frontend_type = type_map.get(db_type, db_type)
    ...
    return {
        ...
        "eventType": frontend_type,
        "alarm_type": frontend_type,
        "type": frontend_type,
        ...
        # Generate title based on alarm_type and description
        "title": _generate_alarm_title(event),
        "description": event.description or f"目标触发 {frontend_type} 规则",
