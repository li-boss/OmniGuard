...
def get_alarm_clip(alarm_id):
    ...
    if not alarm.clip_url:
        return jsonify({"code": 404, "message": "No clip available"}), 404

    clip_path = os.path.join(current_app.root_path, alarm.clip_url.lstrip('/'))
    if not os.path.isfile(clip_path):
        return jsonify({"code": 404, "message": "Clip file not found"}), 404
