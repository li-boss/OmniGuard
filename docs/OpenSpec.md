...
> 文档版本：1.0  
> 基线：分支 `combineMonThirNight`，提交 `60a4fc7a`（2026-07-13 19:34:51 +08:00）  
> 核验日期：2026-07-14  
> 证据范围：当前仓库源代码、目录结构、`README.md`、`docs/`、测试、`ops/Jenkinsfile`、`deploy/nginx-rtmp/README.md`、全部 Git 分支与提交记录。当前根目录未发现 `API_DOCUMENTATION.md`；旧接口说明仅存在于其他副本或历史提交，因此本文以当前已注册路由为准。工作树在分析前无未提交差异。
...
Vue 3 / Element Plus / Pinia
          ...
          ▼
Flask API / JWT / Flask-SocketIO
          │
          ├── API 蓝图 ── 用户、人脸、围栏、告警、日志、日报、大盘、视频流
          │
...
| API | Python、Flask 3.1、Flask-CORS、Flask-JWT-Extended、Flask-SocketIO、Waitress |
...
当前代码存在并已接线：认证与用户、人脸登记/特征库/识别、通行日志、围栏、摄像头流、YOLO 人员检测与跟踪、活体、跌倒、火情、告警 CRUD/截图/录像、WebSocket、钉钉通知、监控大盘、日报/PDF/调度、RTMP 推流、健康检查和 SQLite 字段兼容迁移。
...
- API：`POST /api/faces/register`、`GET /api/faces`、`GET /api/faces/features`、`GET /api/faces/<id>/image`、`DELETE /api/faces/<id>`、`GET/POST /api/access-logs`。
- RetinaFace：`ModelLoader.get_face_detector()` 动态导入 RetinaFace detector；当前本机测试显示安装包导出不兼容。
...
- AccessLog：已知用户识别成功写入 user ID、摄像头、方向、结果和时间；10 秒内去重。
...
- 回放：后端根据 `video_path` 定位受控静态目录并 `send_file(..., conditional=True)`；前端以 blob URL 播放并在关闭时释放 URL。
...
| 分类 | 文件/范围 | 覆盖内容 |
|---|---|---|
| 接口测试 | `test_auth.py`、`test_health.py`、`test_alarm_flow.py`、`test_face_register.py` | 登录/本人、重复注册、健康、围栏-模拟告警-处置、人脸及通行日志 API |
...
- 结果：22 项，20 通过、2 失败，耗时 16.10 秒，29 个 warning。
- 失败 1：`TestCameraPipelineE2E.test_pipeline_e2e_alarm_trigger_and_db_write` 期望告警类型 `electronic_fence`，当前 pipeline 实际产生 `围栏入侵告警`。这是测试契约与实现不一致。
- 失败 2：`FaceRegisterTest.test_face_lifecycle` 使用 1×1 JPEG；当前 RetinaFace 包无法导入预期 `RetinaFaceDetector`，接口正确地以 400 拒绝“无法检测或解析人脸”，与测试原先期待 201 不一致。
- 环境告警：本机缺少 `transformers`、MediaPipe 功能不可用、火情权重缺失；AI 分析回退到规则模式。JWT 测试密钥长度、`datetime.utcnow()` 和 SQLAlchemy `Query.get()` 产生弃用/安全 warning。
- 命令：`npm.cmd run build`（目录 `frontend`）。
- 结果：成功，Vite 转换 3468 个模块并生成 `dist`；Rollup 对 `@vueuse/core` 的 PURE 注释给出非阻塞警告。最大 element chunk 约 922.66 kB（gzip 298.42 kB）。
- Git 工作树：测试/构建后生成物均被忽略，除本文外无源代码差异。

### 6.3 验收矩阵与缺口

| 类别 | 当前证据 | 判定 |
|---|---|---|
| 接口测试 | 认证、健康、告警流程、通行日志多数通过 | 部分通过；人脸录入环境失败 |
...
- ArcFace 业务代码已接入，但仓库未跟踪权重，且本次环境因 RetinaFace 包接口不兼容而未完成真实人脸链路验收。
- 自动化测试未全绿（20/22）；真实 AI 模型、录像、浏览器回放、钉钉、RTMP 和多路长稳没有正式测试报告。
- 前端没有 Vitest 用例，也没有 Playwright/Cypress 等端到端测试。
- 当前仓库缺少根目录 `API_DOCUMENTATION.md`；旧 `docs/spec.md` 与现状存在偏差。

### 7.3 风险

- 依赖/权重风险：RetinaFace 包接口与代码不兼容，Transformers、MediaPipe 和火情权重在本机不可用，关键识别能力可能退化或停用。
...
1. 先锁定并修复 RetinaFace/MediaPipe/Transformers 依赖，提供可复现权重清单与离线校验值，使 22 项后端测试全绿。
...
本文没有把不存在的 MySQL 迁移或未执行的实机测试写成已完成功能；ArcFace 仅按代码接入状态认定，未把缺少权重和真实链路测试的情况写成验收通过。所有结论均受上述仓库基线和本次运行环境约束。
